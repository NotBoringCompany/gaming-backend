...
public static class PvP_GetOpponentData
{
    ...
    [FunctionName("GetOpponentDataFromMasterClient")]
    public static async Task<dynamic> GetOpponentDataFromMasterClient([HttpTrigger(AuthorizationLevel.Function, "get", "post", Route = null)] HttpRequest req, ILogger log)
    {
        //Setup serverApi (Server API to PlayFab)
        FunctionExecutionContext<dynamic> context = JsonConvert.DeserializeObject<FunctionExecutionContext<dynamic>>(await req.ReadAsStringAsync());
        dynamic args = context.FunctionArgument;
        ...
        return $"Get Opponent From Master Client Success! Please Inform your opponent (The PhotonNetwork.MasterClient) that this player has received Data.";
    }
}